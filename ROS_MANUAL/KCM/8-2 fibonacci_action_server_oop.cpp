...
public:
  ...
  rclcpp_action::GoalResponse
	//Goal Request가 올 시 실행되는 callback
	//request가 너무 클 시 REJECT
  ...
  handle_cancel(const std::shared_ptr<GoalHandleFibonacci> goal_handle) {
	...
	//여러 Request에 대응가능한 구현. execute 함수 사용
	// 일반 Action Server시 std::bind(&FBActionServer::execute, this, _1) 같은 형태로
	// Server 생성사 단에 등록
  ...
  void execute(const std::shared_ptr<GoalHandleFibonacci> goal_handle) {
    ...
    const auto goal = goal_handle->get_goal();
		...
		// Feedback, Result는 다음처럼 생성, std::vector 형식을 갖음	
    ...
    for (int i = 1; (i < goal->order) && rclcpp::ok(); ++i) {
      // Check if there is a cancel request
			// cancel탐지하고 goal_handle을 통해 상태를 변경, 루프를 벗어남
			// goal_handle은 여러 상호작용 담당
